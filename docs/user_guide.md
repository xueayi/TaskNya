...
TaskNya 是一个强大的任务监控工具，可以帮助你监控训练任务、文件变化、日志输出等，并通过 Webhook (飞书/AstrBot) 发送通知。
...
- [通知设置](#通知设置)
  - [飞书机器人](#飞书机器人)
  - [通用 Webhook / AstrBot](#通用-webhook--astrbot)
- [高级功能](#高级功能)
  - [变量说明](#变量说明)
  - [操作建议配置](#操作建议配置)
...
### 通用 Webhook / AstrBot
支持发送 HTTP POST 请求到任意地址，特别适配 AstrBot。

#### AstrBot 简易模式
专为 AstrBot 设计的配置界面。
- **UMO 参数**: 格式 `机器人名:消息类型:目标ID`。
- **通知表头**: 消息标题，如 "任务完成"。
- **通知内容**: 消息正文，支持变量。
- **其他内容**: 补充信息。
- **二次元语录**: 开启后会在消息末尾附加一条随机动漫台词。
